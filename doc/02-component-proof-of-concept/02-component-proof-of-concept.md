...
I chose the weight tracking app because it offers a clear, focused set of features that can be implemented incrementally. The app needs to track weights, repetitions, and sets during workouts, which provides a solid basis for building out data structures and methods. This design is also more aligned with my interests in fitness and its utility can be easily tested by real users. The ability to integrate features like historical workout tracking or goal setting could be added later, but for now, focusing on the basic tracking functionality will allow me to create a manageable proof-of-concept that still demonstrates key software design skills.
